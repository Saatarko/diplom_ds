...
from maze_env.maze_build_env import MazeBuilderEnvDFS, CustomTransformerPolicyForBuilder, flatten_trajectories, load_demonstrations, PPOWithImitation, prepare_expert_obs_batch, get_expert_action_from_index, get_expert_action, convert_obs_to_tensor_dict, find_closest_expert_action
...
set_seed(42)
LOAD_PATH = "./generator/generator_agent_dagger_7.zip"
SAVE_PATH = "./generator/generator_agent_dagger_7"
...
def train_generator_agent_with_dagger(dagger=False):
    ...
    env = MazeBuilderEnvDFS(size=7, verbose =0, use_stub_eval=True)

    start_iter = load_dagger_progress()

    if os.path.exists(LOAD_PATH):
        print("Загружаю существующую модель генератора")
        model = PPOWithImitation.load(LOAD_PATH, custom_objects={"policy_class": CustomTransformerPolicyForBuilder})
        model.set_env(env)
    else:
        print("Создаю новую модель генератора")
        model = PPOWithImitation(
...
train_generator_agent_with_dagger(dagger=False)
